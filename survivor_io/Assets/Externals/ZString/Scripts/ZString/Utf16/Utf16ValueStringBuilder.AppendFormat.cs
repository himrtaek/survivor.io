...
namespace Cysharp.Text
{
    public partial struct Utf16ValueStringBuilder
    {
		...
		/// <summary>
		/// boxing/unboxing, garbage도 발생하지만 16개의 인자를 넘겨도 범용적으로 처리할 수 있는 함수
		/// </summary>
		...
		public void AppendFormat(string format, params object[] args)
		{
			...
			for (int i = 0; i < format.Length; i++)
			{
				...
				if (c == '{')
				{
					...
					var argName = indexParse.Index.ToString();
					AppendFormatInternal(args[indexParse.Index], indexParse.Alignment, indexParse.FormatString, argName);
				}
				else if (c == '}')
				{
					if (i + 1 < format.Length && format[i + 1] == '}')
